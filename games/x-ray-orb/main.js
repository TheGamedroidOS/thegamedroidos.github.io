...
function talk(type, one_off = false) {
  ...
  if (type == "hello") {
    comment.chat("hero", [
      ...
      "Click up and up buttons to jump higher.",
    ]);
  ...
  if (type == "door") {
    ...
    for (let i = 0; i < game.effects.length; i++)
      if (effectCoin(game.effects[i].type)) enemy = true;

    comment.chat("hero", [
      "This door is locked.",
      "Collect all the coins to unlock it.",
    ]);
    if (enemy)
      comment.chat("hero", [
        "Note that coins may still be trapped inside the larger enemies.",
      ]);
  ...
  if (type == "orb2") {
    comment.chat("hero", [
      "Oh no! My solar powered jetpack does not work in the dark!",
    ]);
    return;
  }
  if (type == "orb3") {
...
function rotateSmooth(current, goal, base) {
  ...
  if (current > base.max - quart && goal < base.min + quart)
    current -= Math.PI * 2;
  else if (current < base.min + quart && goal > base.max - quart)
    current += Math.PI * 2;
...
function merge(obj1, obj2, gravity, fly = 0, speed_x = 0) {
  ...
  const smallest_x = margin.left < -margin.right ? margin.left : margin.right;
  const smallest_y = margin.top < -margin.bottom ? margin.top : margin.bottom;

  if (
    Math.abs(smallest_x) - speed_x <
    Math.abs(smallest_y) - (smallest_y > 0 ? gravity : fly)
  )
...
function startGame() {
  ...
  write("X Ray Orb", 0.08, 0.45);

  const control = MOBILE
    ? "Use The Arrow Buttons To Operate The Game"
    : "Use The Arrow Keys To Operate The Game";
  ctx.fillStyle = rgb(0, 0, 0, ALPHA);
  write(control, 0.034, 0.6, "");

  const begin = MOBILE
    ? "Click Anywhere To Start"
    : "Press An Arrow Key To Start";
  ctx.fillStyle = rgb(0, 0, 150 + Math.sin(start.time / 10) * 300, ALPHA);
  write(begin, 0.04, 0.7);

  const fake = fakePos(cvs.width / 2, Y + SIZE * 0.2);
  ...
  ctx.fillStyle = "#000";
  fillRect(_fake.x, hero.y + hero.height, fake.x * 2 - _fake.x * 2, 0.03);
...
function update() {
  ...
  if (MOBILE) {
    const point = (x, type, angle) => {
      const size =
        cvs.height < cvs.width ? (cvs.width + cvs.height) / 20 : cvs.width / 10;
      ...
      const draw = (y_offset) => {
        ...
        ctx.translate(
          x + arrow.width / 2,
          cvs.height - size / 2 + y_offset - y_lift
        );
...
function endGame() {
  ...
  write(
    "You took " + mins + minutes + " and " + secs + seconds,
    scale / 2.5,
    cvs.height / 2 + scale * 1.2,
    ""
  );
